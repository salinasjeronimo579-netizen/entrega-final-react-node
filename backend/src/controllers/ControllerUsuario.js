import { registrarUsuario, borrarUsuario, listarUsuarios, modificarUsuario, consultarUsuariosNombre } from "../services/ServicesUsuario.js";

export function crear(req, res) {
    registrarUsuario(req.body)
        .then((response) => {
            res.status(201).json(response)
        })
        .catch((error) => {
            res.status(400).json({ error: error.message })
        })
}

export function eliminar(req, res) {
    borrarUsuario(req.params.id)
        .then((response) => {
            res.status(200).json({ message: "usuario eliminado correctamente" })
        })
        .catch((error) => {
            res.status(400).json({ error: error.message })
        })
}

export function listar(req, res) {
    listarUsuarios()
        .then((response) => {
            res.status(200).json(response)
        })
        .catch((error) => {
            res.status(400).json({ error: error.message })
        })
}

export function actualizar(req, res) {
    modificarUsuario(req.body, req.params.id)
        .then((response)=>{
            res.status(200).json({message: "Usuario editado correctamente."})
        })
        .catch((error)=>{
            res.status(400).json({error: error.message})
        })
}

export function buscarPorNombre(req, res) {
    consultarUsuariosNombre(req.params.nombre)
        .then((response) => {
            res.status(200).json(response)
        })
        .catch((error) => {
            res.status(400).json({ error: error.message })
        })
}